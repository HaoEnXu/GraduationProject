  ...
  <div class="matterPage">
    ...
    <div class="matterContent">
      <div class="topHandleArea">
        ...
        <!-- 上传 -->
        <!--  @click="clickUpLoad" -->
        <span class="Upload-material" @click="clickUpLoad" v-show="this.selectedArr.length==0">上传素材</span>
        <!-- 筛选 -->
      ...
      <el-dialog
        ...
        <div class="searchBody" >
          <ul>
            <li>
              <span class="li-title">素材类型</span>
              <div class="li-content">
                <span :class="choosedTag.baseType == index?'active':''" v-for="(item,index) in baseType" :key="index" @click="basetypeClick(item,index)">{{item.name}}</span>
              </div>
            </li>
            <li>
      ...
      <el-dialog
          ...
          <div class="detailTop">
            ...
            <div class="picture" v-if="choosedItem.type == 0" :style="{backgroundImage:'url('+choosedItem.url+'?x-oss-process=image/resize,m_fill,w_320,h_180'+')'}"></div>
            <div class="video" v-else>
                <video-player  class="video-player vjs-custom-skin"
                  id="video-player"
                  ref="videoPlayer"
                  v-show="choosedItem.type == 1"
                  :playsinline="true"
            ...
            <div class="bottomInfo">
              ...
              <span class="videolength" v-show="this.choosedItem.type == 0">{{choosedItem.width}}*{{choosedItem.height}}</span>
            </div>
          </div>
      ...
      <el-dialog :visible.sync="uploadDialog">
        ...
        <el-upload
          ...
          <el-button type="success" size="small" @click="submitUpload" :disabled="fileList.length == 0">上传到服务器</el-button>
        </el-upload>
      </el-dialog>
      <!-- 修改属性 -->
  ...
  export default {
    ...
    methods:{
      ...
      getShelfData() {
        ...
        this.inputInfo = ''
        const that = this
        this.$request.put("/oms/v1/materials", {
      ...
      editItem(item) {
        ...
        // this.choosedItem.customs!=null?this.aloneCustoms = this.choosedItem.customs.split(','):this.aloneCustoms=''
        this.choosedItem.customs!=null?this.aloneCustoms = this.choosedItem.customs:this.aloneCustoms=''
        // this.choosedItem.industry!=null?this.aloneindustry = this.choosedItem.industry.split(','):''
        this.choosedItem.industry!=null?this.aloneindustry = this.choosedItem.industry:''
        this.choosedItem.type ==1? this.playerOptions.sources[0].src = this.choosedItem.url.split('?')[0]:this.playerOptions.sources[0].src = ''
      },
      // 高级搜索
      Advancedsearch() {
        this.searchBtn = true
      ...
      search() {
        ...
        // 发起请求
        that.$request.put("/oms/v1/materials",that.searchInfo).then(res => {
      ...
      toShelf(item) {
        ...
        console.log(toUpList,'上架数据')
        this.$request
        .patch("/oms/v1/materials/up", toUpList)
        .then(response => {
          console.log(response,'上架')
          response.data.data.forEach(ele => {
            this.unshelfdata.forEach((element,index) => {
              console.log(index)
              if(ele == element.id) {
                console.log(element)
                this.unshelfdata.splice(index,1)
              }
            })
          })
          _this.selectedArr = [];
      ...
      toUnShelf(item) {
        ...
        console.log(toDownList,'下架数据')
          this.$confirm(
            ...
            this.$request
              .put("/oms/v1/materials/down", toDownList)
              .then(response => {
                console.log(response,'下架')
                response.data.data.forEach(ele => {
                  this.shelfdata.forEach((element,index) => {
                    console.log(element)
                    if(ele == element.id) {
                      console.log(element)
      ...
      aloneItemRefresh() {
        ...
        this.$request
        .patch("oms/v1/materials", paramater)
        .then(response => {
          if(this.type == 0) {
            this.getShelfData()
          } else if (this.type == 1) {
      ...
      toDelete(item) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
            ...
            this.$request.delete('/oms/v1/materials',paramater).then(res => {
              console.log(res)
              if(res.status == 200) {
                this.$message({
                  type: 'success',
      ...
      modify() {
        ...
        _this.$request
          .patch("oms/v1/materials/attributes", modiftyarr)
          .then(response => {
      ...
      handleChange(file, fileList) {
        fileList.map((ele,index) => {
          if (
            ele.name.indexOf("MP4") === -1 &&
            ele.name.indexOf("mp4") === -1 &&
            ele.name.indexOf("png") === -1 &&
            ele.name.indexOf("PNG") === -1 &&
            ele.name.indexOf("jpg") === -1 &&
            ele.name.indexOf("JPG") === -1 &&
            ele.name.indexOf("jpeg") === -1 &&
            ele.name.indexOf("JPEG") === -1 &&
            ele.name.indexOf("mov") === -1 &&
            ele.name.indexOf("MOV") === -1 &&
            ele.name.indexOf("quicktime") === -1 &&
            ele.name.indexOf("mkv") === -1 &&
            ele.name.indexOf("MKV") === -1
          ) {
            this.$message({
              message: "文件只能是 jpg,png,mp4,mov,mkv 格式!",
              type: "error",
              duration: 2000
            });
      ...
      submitUpload() {
        ...
        Promise.all(Array.from(this.fileList).map((file,index) => {
            return new Promise((resolve,reject) => {
              if(
                ...
                this.uploadVideo(file,index,loading)
              } else {
                this.uploadPicture(file,index,loading)
              }
            })
        }))
      },
      // 上传视频
      uploadVideo(ele,index,loading) {
          const that = this
          let name = Date.parse(new Date()) + index + '.' + ele.name.split('.')[ele.name.split('.').length - 1]
          let fileType = ele.raw.type.split('video/')[1]==='quicktime'?'mov':ele.raw.type.split('video/')[1]
          that.$request.post('/oms/v1/policy', {
              name,
              type: 'video',
          }).then(res => {
              if(res.status !== 200 || !res || res.data.errorCode!==200) {
                this.$message({
                  type:'error',
                  message:"数据错误"
                })
                  ...
                  that.newUpItem.push(data.fileId) 
                  let formData = new FormData()
                  let name = data.uuid+ '.' + ele.name.split('.')[ele.name.split('.').length - 1]
                  formData.append("key", name)
                  formData.append("policy", data.policy)
                  formData.append("OSSAccessKeyId", data.accessId)
                  formData.append("signature", data.signature)
                  formData.append("callback", data.callbackBase64Str)
                  formData.append("success_action_status", 200)
                  formData.append("file", ele.raw)
                  that.$request.post('https://molto-pixel-res.oss-cn-beijing.aliyuncs.com',formData).then(res => {
                    if(res == undefined || res.status == 203) {
                      that.$notify({
                        title: '出错提示',
                        message:'文件'+ele.name+'上传出错,请检查网络或重新上传',
                        duration: 0
                      });
                      that.uploadNum++
                    ...
                    if(that.uploadNum == that.fileList.length) {
      ...
      // 上传照片
      uploadPicture(file,index,loading) {
        const that = this
        var picWidth = 0
        var picHeight = 0
        ...
        reader.onload = function(e) {
          ...
          image.onload = function() {
            ...
            let name = Date.parse(new Date()) +index+ '.' + file.name.split('.')[file.name.split('.').length - 1]
            that.$request.post('/oms/v1/policy',{
              type: "photo",
              width: picWidth,
              height: picWidth,
              ...
              if (res.status !== 200 || !res || res.data.errorCode !== 200) {
                ...
                that.newUpItem.push(data.fileId)
                let formData = new FormData()
                let name = data.uuid +"." +file.name.split(".")[file.name.split(".").length - 1];
                formData.append("key", name)
                formData.append("policy", data.policy)
                formData.append("OSSAccessKeyId", data.accessId)
                formData.append("signature", data.signature)
